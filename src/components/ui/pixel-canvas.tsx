...
const PixelCanvas = React.forwardRef<HTMLDivElement, PixelCanvasProps>(
  ({ gap, speed, colors, variant, noFocus, style, ...props }, ref) => {
    const pixelCanvasRef = React.useRef<HTMLDivElement>(null);
    ...
    React.useEffect(() => {
      ...
      if (typeof window !== "undefined") {
        ...
        // Create and append the custom element to the div
        if (pixelCanvasRef.current) {
          const pixelCanvasElement = document.createElement("pixel-canvas") as HTMLElement & {
            dataset: DOMStringMap;
          };
          
          // Set data attributes
          if (gap !== undefined) pixelCanvasElement.dataset.gap = gap.toString();
          if (speed !== undefined) pixelCanvasElement.dataset.speed = speed.toString();
          if (colors) pixelCanvasElement.dataset.colors = colors.join(",");
          if (variant) pixelCanvasElement.dataset.variant = variant;
          if (noFocus) pixelCanvasElement.dataset.noFocus = "";
          ...
          pixelCanvasRef.current.innerHTML = '';
          pixelCanvasRef.current.appendChild(pixelCanvasElement);
        }
      }
    }, [gap, speed, colors, variant, noFocus, style]);

    // Use React.useImperativeHandle to forward methods/properties to the ref
    React.useImperativeHandle(ref, () => pixelCanvasRef.current as HTMLDivElement);
