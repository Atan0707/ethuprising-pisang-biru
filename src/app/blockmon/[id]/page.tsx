...
const GET_BLOCKMON_BATTLES = gql`
  query GetBlockmonBattles($tokenId: String!) {
    battleCompleteds(where: {tokenId: $tokenId}, first: 100, orderBy: blockTimestamp, orderDirection: desc) {
      id
      tokenId
      opponentId
      won
...
const GET_BLOCKMON_LEVELS = gql`
  query GetBlockmonLevels($tokenId: String!) {
    pokemonLeveledUps(where: {tokenId: $tokenId}, first: 100, orderBy: blockTimestamp, orderDirection: desc) {
      id
      tokenId
      newLevel
      blockTimestamp
...
export default function BlockmonDetailsPage() {
  ...
  if (error) {
    return (
      <div className="container mx-auto px-4 py-16 text-center">
        ...
        <Link href="/leaderboard" className="mt-8 inline-block bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">
          Back to Leaderboard
        </Link>
      </div>
    );
    ...
    return (
      <div className="container mx-auto px-4 py-16 text-center">
        <div className="bg-yellow-100 border border-yellow-400 text-yellow-700 px-4 py-3 rounded relative" role="alert">
          ...
          <span className="block sm:inline"> Blockmon #{tokenId} not found.</span>
        </div>
        <Link href="/leaderboard" className="mt-8 inline-block bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">
          Back to Leaderboard
        </Link>
  ...
  const winRate = blockmonData.battleCount > 0 
    ? Math.round((blockmonData.battleWins / blockmonData.battleCount) * 100) 
    : 0;

  return (
    <div className="container mx-auto px-4 py-8">
      <Link href="/leaderboard" className="inline-flex items-center mb-6 text-blue-600 hover:text-blue-800">
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-lg overflow-hidden">
        ...
        <div className="p-6">
          {activeTab === 'stats' && (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="bg-gray-50 dark:bg-gray-700 rounded-lg p-6">
                <h3 className="text-lg font-semibold mb-4 text-gray-800 dark:text-white">Combat Stats</h3>
                <div className="space-y-4">
                  <div>
                    <div className="flex justify-between mb-1">
                      <span className="text-sm font-medium text-gray-700 dark:text-gray-300">HP</span>
                      <span className="text-sm font-medium text-gray-700 dark:text-gray-300">{blockmonData.hp}</span>
                    </div>
                    <div className="w-full bg-gray-200 rounded-full h-2.5 dark:bg-gray-600">
                      <div className="bg-green-500 h-2.5 rounded-full" style={{ width: `${Math.min(100, (blockmonData.hp / 200) * 100)}%` }}></div>
                    </div>
                  </div>
                  <div>
                    <div className="flex justify-between mb-1">
                      <span className="text-sm font-medium text-gray-700 dark:text-gray-300">Attack</span>
                      <span className="text-sm font-medium text-gray-700 dark:text-gray-300">{blockmonData.baseDamage}</span>
                    </div>
                    <div className="w-full bg-gray-200 rounded-full h-2.5 dark:bg-gray-600">
                      <div className="bg-red-500 h-2.5 rounded-full" style={{ width: `${Math.min(100, (blockmonData.baseDamage / 100) * 100)}%` }}></div>
                    </div>
                  </div>
                  <div>
                    <div className="flex justify-between mb-1">
                      <span className="text-sm font-medium text-gray-700 dark:text-gray-300">Experience</span>
                      <span className="text-sm font-medium text-gray-700 dark:text-gray-300">{blockmonData.experience}</span>
                    </div>
                    <div className="w-full bg-gray-200 rounded-full h-2.5 dark:bg-gray-600">
                      <div className="bg-blue-500 h-2.5 rounded-full" style={{ width: `${Math.min(100, (blockmonData.experience % 100))}%` }}></div>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-gray-50 dark:bg-gray-700 rounded-lg p-6">
                <h3 className="text-lg font-semibold mb-4 text-gray-800 dark:text-white">General Info</h3>
                <div className="space-y-3">
                  <div className="flex justify-between">
                    <span className="text-gray-600 dark:text-gray-400">Birth Time</span>
                    <span className="font-medium text-gray-800 dark:text-white">
                      {new Date(blockmonData.birthTime * 1000).toLocaleDateString()}
                    </span>
                  </div>
                  <div className="flex justify-between">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium text-gray-800 dark:text-white">
                      {blockmonData.lastBattleTime > 0 
                        ? new Date(blockmonData.lastBattleTime * 1000).toLocaleDateString() 
                        : 'Never battled'}
                    </span>
                  </div>
                  <div className="flex justify-between">
          ...
          {activeTab === 'battles' && (
            <div>
              <h3 className="text-lg font-semibold mb-4 text-gray-800 dark:text-white">Battle History</h3>
                ...
                <div className="overflow-x-auto">
                  <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                    ...
                    <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                      {battleData.battleCompleteds.map((battle: BattleRecord) => (
                        <tr key={battle.id}>
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">
                            {new Date(parseInt(battle.blockTimestamp) * 1000).toLocaleDateString()}
                          </td>
                          <td className="px-6 py-4 whitespace-nowrap">
                            <Link href={`/blockmon/${battle.opponentId}`} className="text-sm text-blue-600 hover:text-blue-800 dark:text-blue-400 dark:hover:text-blue-300">
                              #{battle.opponentId}
                            </Link>
                          </td>
                          <td className="px-6 py-4 whitespace-nowrap">
          ...
          {activeTab === 'levels' && (
            <div>
              <h3 className="text-lg font-semibold mb-4 text-gray-800 dark:text-white">Level History</h3>
                  ...
                  <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                    ...
                    <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                      {levelData.pokemonLeveledUps.map((level: LevelRecord) => (
                        <tr key={level.id}>
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">
                            {new Date(parseInt(level.blockTimestamp) * 1000).toLocaleDateString()}
                          </td>
                          <td className="px-6 py-4 whitespace-nowrap">
                            <div className="text-sm font-semibold text-blue-600 dark:text-blue-400">Level {level.newLevel}</div>
                          </td>
                        </tr>
                      ))}
                    </tbody>
