...
export default function ListNFTPage() {
      ...
      try {
        ...
        if (ownedTokens.length === 0) {
          ...
          toast.info('No Blockmons found', {
            description: 'You don\'t own any Blockmons yet. Try minting one first!',
            icon: '🔍',
          });
        } else {
        ...
        toast.error(`Failed to fetch your Blockmons: ${(error as Error).message}`);
        setIsLoading(false);
      }
    };
  ...
  const startNfcScan = async () => {
    ...
    try {
      ...
      // Extract the Blockmon ID from the NFC data
      // Format could be "blockmon:123" or just "123"
      const blockmonId = nfcData.includes(':') ? parseInt(nfcData.split(':')[1]) : parseInt(nfcData);
      
      if (isNaN(blockmonId)) {
        toast.dismiss(scanToastId);
        toast.error('Invalid NFC Card', {
          description: 'The NFC card does not contain a valid Blockmon ID.',
          icon: '❌',
        });
        setIsScanning(false);
        return;
      ...
      // Verify the scanned NFT exists in the user's collection
      const ownedNFT = ownedNFTs.find(nft => nft.id === blockmonId);
      
      if (!ownedNFT) {
        toast.dismiss(scanToastId);
        toast.error('Ownership Verification Failed', {
          description: 'You do not own the Blockmon on this NFC card.',
          icon: '❌',
        });
        setIsScanning(false);
        return;
      ...
      toast.success('Blockmon Verified', {
        description: `Successfully verified Blockmon #${blockmonId}: ${ownedNFT.name}`,
        icon: '✅',
      });
      
      // Set the scanned NFT and automatically select it
      setScannedNFT(blockmonId);
      setSelectedNFT(blockmonId);
      setIsScanning(false);
    } catch (error) {
  ...
  const handleSubmitListing = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await listNFT(selectedNFT, price, walletProvider as import('ethers').Eip1193Provider);
      
      toast.dismiss(loadingToast);
      toast.success(`Successfully listed Blockmon #${selectedNFT} for ${price} ETH!`);
      ...
      toast.error('Failed to list Blockmon', {
        description: 'There was an error listing your Blockmon. Please try again.',
      });
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {ownedNFTs.length === 0 ? (
          <div className="text-center py-12 bg-white dark:bg-gray-800 rounded-lg shadow-md">
            <h3 className="mt-2 text-lg font-medium text-gray-900 dark:text-white">No Blockmon available</h3>
                  ...
                  >
                    <div className="relative h-40 w-full bg-gray-200 dark:bg-gray-700">
                      {nft.tokenURI ? (
                        ...
                        <div className="h-full w-full flex items-center justify-center">
                          <span className="text-6xl">{attributeMap[nft.attribute].icon}</span>
                        </div>
                      )}
                    ...
                    <div className="p-3">
                      <div className="flex justify-between items-start">
                        <h3 className="text-md font-semibold text-gray-900 dark:text-white">{nft.name}</h3>
                        <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${attributeMap[nft.attribute].bgColor} ${attributeMap[nft.attribute].color}`}>
                          {attributeMap[nft.attribute].name}
                        </span>
                      </div>
                      <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">
                        Rarity: <span className="font-medium">{rarityMap[nft.rarity].name}</span>
                      </p>
                      <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">
                ...
                <div className="mt-8">
                  ...
                  {nfcSupported === false ? (
                    <div className="bg-yellow-100 dark:bg-yellow-900/30 text-yellow-700 dark:text-yellow-300 p-4 rounded-lg mb-4">
                      ...
                      <p className="mt-1">Your device or browser doesn&apos;t support NFC scanning. Please use a compatible device (like an Android phone with Chrome).</p>
                    </div>
                  ) : (
                    <div className="flex flex-col md:flex-row items-center gap-6 p-4 bg-gray-50 dark:bg-gray-700 rounded-lg">
                      ...
                      <div className="flex-1">
                        <p className="mb-4 text-gray-600 dark:text-gray-300">
                          To verify ownership, please scan the NFC card associated with your Blockmon. This ensures that only the rightful owner can list the Blockmon for sale.
                        </p>
                  ...
                  <div className="flex flex-col sm:flex-row gap-4">
                    <div className="flex-grow">
                      ...
                      <p className="mt-2 text-xs text-gray-500 dark:text-gray-400">
                        Set a fair price for your Blockmon. A marketplace fee of 2.5% will be applied to the sale.
                      </p>
                    </div>
                    ...
                    <div className="flex items-end">
                      <button
                        ...
                        className={`w-full sm:w-auto px-6 py-3 border border-transparent text-base font-medium rounded-md shadow-sm text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 ${
                          (!price || isSubmitting) ? 'opacity-50 cursor-not-allowed' : ''
                        }`}
