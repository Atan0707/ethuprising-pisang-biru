...
interface NDEFReader extends EventTarget {
  ...
  write(
    message: NDEFMessage | NDEFRecord[] | NDEFRecord,
  ...
  try {
    ...
    await ndef.write(
      {
        records: [
          ...
          {
            recordType: "url",
            data: `https://blocknogotchi.fun/blockmon/${id}` as unknown as BufferSource,
          },
        ],
      },
    ...
    if (error instanceof Error) {
      if (error.name === "NotAllowedError") {
        ...
        throw new Error(
          "NFC tag does not support NDEF or NDEF cannot be written to this tag"
        );
...
export const readFromNfcTag = async (options?: {
  timeoutMs?: number;
}): Promise<string> => {
  if (!isNfcSupported()) {
    throw new Error("NFC is not supported on this device or browser");
